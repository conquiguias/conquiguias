(function (global) {
  const AD_NETWORK_ENABLED_DEFAULT = false;

  function initResponsiveAds(options) {
    const config = options || {};
    const adNetworkEnabled =
      config.enabled ?? global.AD_NETWORK_ENABLED ?? AD_NETWORK_ENABLED_DEFAULT;
    ...
    if (!adNetworkEnabled) {
      ...
      mobile.shell.hidden = true;
      desktop.shell.setAttribute("aria-hidden", "true");
      mobile.shell.setAttribute("aria-hidden", "true");
      document.body.classList.remove(bodyMobileClass);
      return function cleanupDisabledAds() {};
    }
