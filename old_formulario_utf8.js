// Cache en memoria simple para Serverless (persiste mientras la instancia est├® caliente)
const memoryCache = new Map();
const CACHE_TTL = 60 * 1000; // 1 minuto por defecto para datos din├ímicos
const CACHE_TTL_STATIC = 5 * 60 * 1000; // 5 minutos para datos est├íticos (im├ígenes, evaluaciones)

module.exports = async function handler(req, res) {
  const { action } = req.query;

  // Configuraci├│n com├║n - Repositorio SOLO para datos (evita deploys en Vercel)
  const repo = "conquiguias/conquiguias-data";

  try {
    // Manejo de m├®todos permitidos
    if (req.method === "GET") {
      switch (action) {
        ...
        case "listarFormulariosPendientes":
          ...
          break;
        default:
          res.status(400).json({ error: `Acci├│n GET no v├ílida: ${action}` });
          break;
      }
      ...
      switch (action) {
        ...
        case "obtenerEstadoUsuario":
          // Nota: Aunque es una lectura, usa POST porque env├¡a datos sensibles (email) en el body
        ...
        case "eliminarTareasPDF":
          ...
          break;
        default:
          res.status(400).json({ error: `Acci├│n POST no v├ílida: ${action}` });
          break;
      }
    } else {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (resp.ok) {
        ...
        // Inicializar vac├¡o si no existe
...
async function handleActualizarEstadoAsistencia(req, res, repo) {
  ...
  try {
    const result = await updateGitHubJSON(
      ...
      async (formularios) => {
        if (!formularios[id]) {
          // Si no existe, no hacemos cambios, esto causar├í que result.ok sea false o se maneje abajo
          return null; // updateGitHubJSON manejar├í esto como skipped
        }
...
async function handleGuardar(req, res, repo) {
  console.log("Recibiendo petici├│n guardar (v2 - recuperaci├│n)");
  if (req.method !== "POST") return res.status(405).send("M├®todo no permitido");
  ...
  // 1. Verificar si la asistencia est├í activa (Server-side check)
  try {
    ...
    if (!activeState) {
      return res
        .status(403)
        .send("ÔØî La asistencia no est├í activa en este momento.");
    }
  } catch (e) {
    console.error("Error verificando estado activo:", e);
    return res.status(500).send("ÔØî Error verificando estado de asistencia.");
  }
  ...
  try {
    ...
    const result = await updateGitHubJSON(
      ...
      async (registros) => {
        ...
        // Verificar si ya existe el registro (por ID y n├║mero)
        // Nota: Si hubo sincronizaci├│n arriba, ahora coincidir├í aqu├¡
        ...
        if (asistenciaNumero > 1) {
          ...
          if (!tienePrevia) {
            throw new Error(
              `ÔØî Debes completar la asistencia ${asistenciaNumero - 1} antes de registrar la ${asistenciaNumero}`,
            );
          }
        }
    ...
    // Si hubo reemplazo de ID, intentar actualizar tambi├®n los resultados de ex├ímenes
    if (visitantesReemplazados.size > 0) {
      try {
        ...
        await updateGitHubJSON(
          repo,
          archivoResultados,
          `[skip ci] Sincronizaci├│n de examen para ID actualizado: ${visitanteId}`,
          async (resultados) => {
          ...
          console.log("No se pudo sincronizar examenes o no exist├¡an.");
        });
      } catch (e) {
        // Ignorar
      }
    ...
    if (result.ok) {
      ...
      return res.status(200).json({
        ok: true,
        message: "Ô£à Asistencia registrada correctamente.",
        correo: correo, // Importante para persistencia al recuperar
      });
    }
  } catch (err) {
    if (err.message.includes("ÔØî"))
      return res.status(400).json({ error: err.message });
    console.error("Error en handleGuardar:", err);
    return res.status(500).json({ error: "ÔØî Error al procesar asistencia." });
  }
}
...
async function handleGuardarEvaluacion(req, res, repo) {
  ...
  try {
    const result = await updateGitHubJSON(
      repo,
      archivo,
      `[skip vercel] Evaluaci├│n creada/actualizada para formulario ${id}`,
      async () => {
        // En este caso, sobrescribimos siempre con la nueva evaluaci├│n
        // No necesitamos leer la anterior, as├¡ que ignoramos el argumento
        return evaluation;
      },
    );
    ...
    if (result.ok) {
      res
        .status(200)
        .json({ ok: true, message: "Ô£à Evaluaci├│n guardada correctamente." });
    }
  } catch (err) {
    console.error(err);
    res
      .status(500)
      .json({ error: err.message || "Error al guardar evaluaci├│n" });
  }
}
...
async function handleGuardarFormulario(req, res, repo) {
  ...
  try {
    ...
    const result = await updateGitHubJSON(
      ...
      async (data) => {
        if (data[id]) {
          throw new Error(`ÔØî El formulario con ID '${id}' ya existe.`);
        }
      ...
      try {
        ...
        const guardarEvaluacion = await fetch(
          `https://api.github.com/repos/${repo}/contents/${archivoEvaluacion}`,
          {
            ...
            body: JSON.stringify({
              message: `[skip vercel] Evaluaci├│n creada para formulario ${id}`,
...
async function handleGuardarResultadoExamen(req, res, repo) {
  ...
  try {
    ...
    if (result.ok) {
      res.status(200).json({
        ok: true,
        message: "Ô£à Examen enviado correctamente.",
        puntaje: puntaje,
      });
    }
  } catch (e) {
    console.error(e);
    res.status(500).send("ÔØî Error al guardar resultado del examen.");
  }
}
...
async function handleLimpiarFormulariosVencidos(req, res, repo) {
  ...
  try {
    ...
    if (formulariosVencidos.length === 0) {
      return res
        .status(200)
        .json({ mensaje: "Ô£à No hay formularios vencidos" });
    }
    ...
    const result = await updateGitHubJSON(
      repo,
      archivoFormularios,
      `[skip vercel] ÔÅ│ Eliminar formularios vencidos (${formulariosVencidos.join(", ")})`,
      async (formularios) => {
        // Re-verificar vencimientos sobre la data m├ís fresca
        const ahora = new Date();
        const vigentes = {};
    ...
    for (const id of formulariosVencidos) {
      ...
      if (archivoRes.ok) {
        ...
        await fetch(`https://api.github.com/repos/${repo}/contents/${ruta}`, {
          ...
          body: JSON.stringify({
            message: `[skip vercel] ÔÅ│ Eliminar respuestas de formulario vencido ${id}`,
    ...
    res.status(200).json({
      mensaje: `­ƒº╣ Formularios vencidos eliminados: ${formulariosVencidos.join(", ")}`,
    ...
    res
      .status(500)
      .json({ error: "ÔØî Error al limpiar formularios vencidos." });
  }
}
...
async function handleListarFormularios(req, res, repo) {
  ...
  try {
    ...
    console.error(err);
    res.status(500).json({ error: "ÔØî Error al obtener los formularios." });
  }
}
...
async function handleListarImagenes(req, res, repo) {
  ...
  if (!carpeta || (carpeta !== "especialidades" && carpeta !== "firmas")) {
    return res.status(400).json({ error: "Carpeta no v├ílida" });
  }
  ...
  if (memoryCache.has(cacheKey)) {
    ...
    if (Date.now() - timestamp < CACHE_TTL_STATIC) {
      console.log(`[CACHE] Sirviendo im├ígenes ${carpeta} desde memoria`);
      res.setHeader(
        "Cache-Control",
  ...
  try {
    ...
    if (respuesta.ok) {
      ...
      if (respuesta.status === 404) {
        // Carpeta no encontrada o vac├¡a -> devolver array vac├¡o
        // Cachear incluso si est├í vac├¡o para evitar martilleo
        res.setHeader(
          "Cache-Control",
          "public, s-maxage=3600, stale-while-revalidate=86400",
        );
      ...
      if (respuesta.status === 403) {
        return res.status(429).json({
          error: "L├¡mite de peticiones a GitHub excedido. Intente m├ís tarde.",
        });
      }
    ...
    console.error("Error al listar im├ígenes:", err);
    res.status(500).json({ error: "Error al listar im├ígenes: " + err.message });
  }
}
...
async function handleObtenerEvaluacion(req, res, repo) {
  ...
  if (memoryCache.has(cacheKey)) {
    const { data, timestamp } = memoryCache.get(cacheKey);
    if (Date.now() - timestamp < CACHE_TTL_STATIC) {
      console.log(`[CACHE] Sirviendo evaluaci├│n ${id} desde memoria`);
      return res.status(200).json(data);
    }
  }
...
async function handleSubirImagen(req, res, repo) {
  ...
  try {
    ...
    if (guardar.ok) {
      ...
      res.status(200).json({
        ok: true,
        message: "Ô£à Imagen subida correctamente",
        url: urlImagen,
      });
    } else {
      const error = await guardar.json();
      // Si el error es SHA needed y no lo ten├¡amos (race condition?), intentar una vez m├ís?
...
async function handleVerRespuestas(req, res, repo) {
  ...
  try {
    ...
    // Obtener resultados de ex├ímenes si existen
...
async function handleEliminarFormulario(req, res, repo) {
  ...
  try {
    // 1. Eliminar del ├¡ndice (formularios.json)
    const result = await updateGitHubJSON(
      repo,
      archivoFormularios,
      ...
      async (formularios) => {
        ...
        // Copiar para no mutar directo si fuera referencia (aunque aqu├¡ se pasa objeto nuevo)
    ...
    if (!result.ok && !result.skipped)
      throw new Error("Error al actualizar ├¡ndice de formularios");
    ...
    // Intentar borrar tambi├®n la carpeta (GitHub API no borra carpetas vac├¡as autom├íticamente a menos que se borren todos los archivos)
    // Pero como borramos archivos espec├¡ficos, si eran los ├║nicos, la carpeta desaparece 'virtualmente'.
    ...
    for (const ruta of rutasBorrar) {
      try {
        ...
        console.warn(`No se pudo borrar ${ruta} (tal vez no exist├¡a):`, e);
      }
    }
...
async function handleEliminarImagen(req, res, repo) {
  ...
  if (carpeta !== "especialidades" && carpeta !== "firmas") {
    return res.status(400).json({ error: "Carpeta inv├ílida" });
  }
  ...
  try {
    ...
    res.status(500).json({ error: "Error al procesar eliminaci├│n" });
  }
}

// Handler para subirTarea (Alumno) - VERSI├ôN ACTUALIZADA (Usa Email)
async function handleSubirTarea(req, res, repo) {
  if (req.method !== "POST")
    return res.status(405).json({ error: "M├®todo no permitido" });

  // Ahora esperamos 'email' tambi├®n
  ...
  if (!identificador) {
    return res.status(400).json({
      error: "Se requiere un correo electr├│nico o ID para subir la tarea.",
    });
  }
  ...
  if (contenido.length > 5242880)
    return res
      .status(400)
      .json({ error: "El archivo excede el l├¡mite de 5MB" });
  if (!nombreArchivo.toLowerCase().endsWith(".pdf"))
    return res.status(400).json({ error: "Solo se permiten archivos PDF" });

  // Sanitizar identificador para nombre de archivo (reemplazar caracteres inv├ílidos)
...
async function handleCalificarTareas(req, res, repo) {
  ...
  try {
    // Sobrescribir el JSON de tareas con la versi├│n actualizada (que incluye notas)
    await updateGitHubJSON(
      repo,
      path,
      `[skip vercel] Calificaci├│n tareas ${id}`,
      async () => tareas,
    );
...
async function handleListarEntregas(req, res, repo) {
  ...
  try {
    ...
    if (!resp.ok) return res.status(200).json({}); // Si no existe, devolver vac├¡o
...
// Handler consolidado para el Dashboard del Usuario (Tareas y Ex├ímenes)
...
async function handleListarArchivosPDF(req, res, repo) {
  ...
  try {
    // Listar contenido de la carpeta ra├¡z de tareas
    const resp = await fetch(
    ...
    if (!resp.ok) {
      if (resp.status === 404) return res.status(200).json([]); // Carpeta vac├¡a o no existe
...
async function handleEliminarTodasTareasPDF(req, res, repo) {
  ...
  // Recibir lista de archivos a borrar para hacerlo m├ís eficiente? No, el backend deber├¡a saber qu├® borrar.
  // Pero GitHub API requiere DELETE file por file con SHA.
  // As├¡ que reutilizamos l├│gica de listar para obtener SHAs y borrar.
  ...
  try {
    ...
    if (!treeResp.ok)
      throw new Error("No se pudo obtener el ├írbol de archivos");
    ...
    console.error(err);
    res.status(500).json({ error: "Error durante la eliminaci├│n masiva" });
  }
}
      ...
      if (form.tarea && form.tarea.activa) {
        ...
        try {
          ...
          if (rT.ok) {
            ...
            // (O podr├¡a considerarse 0 entregas)
          }
        } catch (e) {
